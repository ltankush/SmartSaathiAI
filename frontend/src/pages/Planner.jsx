...
import { Target, TrendingUp, Calendar, PiggyBank, Sliders } from 'lucide-react'
...
function localFireCalc({ current_age, monthly_income, monthly_expenses, current_savings, fire_age, inflation_rate, equity_return }) {
  ...
  for (const yr of [1, 3, 5, 10, years_to_fire]) {
    if (yr <= years_to_fire) {
      ...
      milestones.push({ year: yr, age: current_age + yr, corpus: Math.round(corpus_at) })
    }
  }

  return { fire_corpus_needed: Math.round(fire_corpus), monthly_sip_needed: sip_needed, years_to_fire, milestones }
}
...
export default function Planner() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setWhatIf({ fire_age: form.fire_age, equity_return: form.equity_return, inflation_rate: form.inflation_rate })
  ...
  const whatIfResult = useMemo(() => {
    ...
    return localFireCalc({ ...form, fire_age: whatIf.fire_age, inflation_rate: whatIf.inflation_rate, equity_return: whatIf.equity_return })
  }, [whatIf, form, result])

  const delta = useMemo(() => {
    if (!result || !whatIfResult) return null
    return { sip: whatIfResult.monthly_sip_needed - result.plan.monthly_sip_needed, years: whatIfResult.years_to_fire - result.plan.years_to_fire }
  }, [result, whatIfResult])

  const chartData = result ? {
    labels: result.chart_data.labels.map((a) => `Age ${a}`),
    datasets: [
      { label: 'Your corpus', data: result.chart_data.corpus, borderColor: '#13b88e', backgroundColor: 'rgba(19,184,142,0.08)', fill: true, tension: 0.4, pointBackgroundColor: '#13b88e', pointRadius: 5 },
      { label: 'FIRE target', data: result.chart_data.target, borderColor: '#f59e0b', borderDash: [6, 4], backgroundColor: 'transparent', fill: false, tension: 0, pointRadius: 0 },
    ],
  } : null

  const chartOpts = {
    responsive: true, maintainAspectRatio: false,
    plugins: { legend: { labels: { color: '#8b949e' } }, tooltip: { backgroundColor: '#161b22', borderColor: '#30363d', borderWidth: 1, callbacks: { label: (ctx) => ` ${formatINR(ctx.parsed.y)}` } } },
    scales: { x: { ticks: { color: '#484f58' }, grid: { color: '#21262d' } }, y: { ticks: { color: '#484f58', callback: (v) => formatINR(v) }, grid: { color: '#21262d' } } },
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="card">
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={submit} className="btn-primary w-full flex items-center justify-center gap-2">
          ...
          {result ? (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card"
                ...
                <button onClick={() => setShowWhatIf(!showWhatIf)} className="w-full flex items-center justify-between">
                  <div className="flex items-center gap-2"><Sliders size={16} className="text-purple-400" /><span className="text-sm font-semibold text-white">What-If Simulator</span></div>
                ...
                {showWhatIf && whatIf && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4 pt-4 border-t border-white/5">
                    <SliderInput label="What if I retire at..." min={form.current_age + 3} max={65} value={whatIf.fire_age} onChange={(v) => setWhatIf(p => ({ ...p, fire_age: v }))} format={(v) => `${v} yrs`} />
                    <SliderInput label="What if returns are..." min={6} max={20} value={whatIf.equity_return} onChange={(v) => setWhatIf(p => ({ ...p, equity_return: v }))} format={(v) => `${v}%`} />
                    <SliderInput label="What if inflation is..." min={3} max={12} value={whatIf.inflation_rate} onChange={(v) => setWhatIf(p => ({ ...p, inflation_rate: v }))} format={(v) => `${v}%`} />
                    {delta && whatIfResult && (
                      <div className="mt-3 p-3 rounded-xl bg-purple-500/5 border border-purple-500/20">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div><span className="text-[#8b949e]">New corpus: </span><span className="text-white font-medium">{formatINR(whatIfResult.fire_corpus_needed)}</span></div>
                          <div><span className="text-[#8b949e]">New SIP: </span><span className="text-white font-medium">{formatINR(whatIfResult.monthly_sip_needed)}</span></div>
                          <div><span className="text-[#8b949e]">SIP change: </span><span className={delta.sip >= 0 ? 'text-red-400' : 'text-brand-400'}>{delta.sip >= 0 ? '+' : ''}{formatINR(delta.sip)}/mo</span></div>
                          <div><span className="text-[#8b949e]">Years: </span><span className="text-amber-400">{whatIfResult.years_to_fire} yrs</span></div>
                        </div>
                      </div>
                    )}
                  </motion.div>
              ...
              <div className="card">
                ...
                {Object.entries(result.allocation).map(([name, info]) => (
                  <div key={name} className="mb-3 last:mb-0">
                    <div className="flex justify-between text-xs mb-1"><span className="text-white font-medium">{name}</span><span className="text-brand-400">{formatINR(info.monthly)}/mo</span></div>
              ...
              <div className="card">
                ...
                {INDEX_FUNDS.slice(0, 3).map((fund, i) => (
                  <motion.div key={fund.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                    className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                    <div><p className="text-xs font-medium text-white">{fund.name}</p><p className="text-[10px] text-[#484f58]">{fund.type} - {fund.category} - ER: {fund.expense_ratio}%</p></div>
                    <div className="text-right"><p className="text-xs font-bold text-brand-400">{fund.returns_5yr}%</p><p className="text-[10px] text-[#484f58]">5yr return</p></div>
                  </motion.div>
                ))}
                <div className="mt-3 pt-2 border-t border-white/5">
                  {SAFE_INSTRUMENTS.slice(0, 2).map((inst) => (
                    <div key={inst.name} className="flex justify-between py-1.5 text-xs"><span className="text-[#8b949e]">{inst.name}</span><span className="text-brand-400 font-medium">{inst.current_rate}%</span></div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-[#484f58]"><Target size={40} className="mx-auto mb-3 opacity-30" /><p className="text-sm">Fill in your details and click Calculate</p></div>
            </div>
          )}
        </div>
      </div>
